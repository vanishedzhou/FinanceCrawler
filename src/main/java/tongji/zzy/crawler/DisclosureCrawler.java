...
public class DisclosureCrawler implements PageProcessor {
	...
	public void process(Page page) {
		if(page.getUrl().regex(LIST_URL).match()) {
			...
			//find companyName and companyCode from content text(first n characters) using regular expression 
			...
			} else {System.out.println("companyCode and companyName not found in the content text.");}
...
//			.run();
		...
		for(DisclosureCategory dc : DisclosureCategory.values()) {
			...
			Spider.create(new DisclosureCrawler())
				.addUrl(getSpecifiedDisclosureCategoryUrl(dc.toString()))
				.run();
		}
