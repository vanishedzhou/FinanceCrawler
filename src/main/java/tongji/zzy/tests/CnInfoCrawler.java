...
public class CnInfoCrawler implements PageProcessor {
	...
	public void process(Page page) {
		...
		// page.putField("companyCode",
		// page.getHtml().xpath("//ul[@id='ul_a_latest']/li[1]/div[@class='t1']/text()"));
		page.putField("Name", page.getHtml().xpath("html/body/div[@class='main-content']//h1/span[1]/text()"));
		page.putField("starredNumber",
				page.getHtml().xpath("html/body//*[@id='js-pjax-container']/div/div/div[1]/div/a[2]/strong/text()"));
		page.putField("followingNumber",
				page.getHtml().xpath("html/body//*[@id='js-pjax-container']/div/div/div[1]/div/a[3]/strong/text()")
						.toString().trim());
		System.out.println(page.getResultItems().get("followingNumber").toString());

		// for(int i=0; i<itemName.length; i++) {
		// page.putField(itemName[i],
		// page.getHtml().xpath(itemNameXPathMap.get(itemName[i])));
		// }

	}

	public static void main(String[] args) {
		Spider.create(new CnInfoCrawler()).addUrl("https://github.com/vanishedzhou").addPipeline(new ConsolePipeline())
				// .addPipeline(new FilePipeline("results"))
				.addPipeline(new JsonFilePipeline("results")).thread(1).run();
